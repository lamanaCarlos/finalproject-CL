...
export const BuyerOrderDetailPage = () => {
  ...
  return (
    <DashboardLayout>
      <div className="max-w-4xl mx-auto">
        {/* Back Button */}
        <Link to="/buyer/orders" className="inline-flex items-center gap-2 text-gray-600 hover:text-gray-900 mb-6">
            ...
            <Card padding="lg">
              ...
              <div className="flex flex-col md:flex-row gap-6">
                ...
                <div className="flex-1">
                  ...
                  <Link to={`/artist/${typeof order.artistId === 'object' ? order.artistId._id : order.artistId}`}>
                    <p className="text-primary-600 hover:underline">{artistName}</p>
                  </Link>
                </div>
              </div>
            </Card>
              ...
              <div className="space-y-4">
                ...
                <div className="border-t pt-4">
                  ...
                  {order.shippingInfo?.shippingCost && (
                    <div className="flex items-center justify-between mb-2">
                      <span className="text-gray-600">{t('order.shippingCost') || 'Envío'}</span>
                      <span className="font-semibold">${order.shippingInfo.shippingCost}</span>
                    </div>
                  )}
                  <div className="flex items-center justify-between text-lg font-bold pt-2 border-t">
                    <span>{t('order.total') || 'Total'}</span>
                    <span className="text-primary-600">
                      ${order.price + (order.shippingInfo?.shippingCost || 0)}
                    </span>
                  </div>
                </div>
              </div>
